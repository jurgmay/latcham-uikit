four51.app.directive('ordershipping', [
	'Order',
	'Shipper',
	'Address',
	'AddressList',
	function (Order, Shipper, Address, AddressList) {
		var obj = {
			restrict: 'AE',
				...
				function ($scope) {
					...
					AddressList.shipping(function (list) {
						...
						if ($scope.currentOrder) {
							...
							if ($scope.isEditforApproval) {
								if (!AddressList.contains($scope.currentOrder.ShipAddress)) $scope.shipaddresses.push($scope.currentOrder.ShipAddress);
							}
						}
					});
					...
					$scope.$on('event:AddressSaved', function (event, address) {
						if (address.IsShipping) {
							...
							if (!$scope.shipToMultipleAddresses) $scope.setShipAddressAtOrderLevel();
							$scope.shipaddressform = false;
						}
					...
					var saveChanges = function (callback, error) {
						...
						Order.save(
							...
							function (ex) {
								if (auto) $scope.currentOrder.ExternalID = auto;
					...
					Shipper.query($scope.currentOrder, function (list) {
						...
						angular.forEach(list, function (s) {
							if (!exists && $scope.currentOrder.LineItems[0].ShipperID == s.ID) exists = true;
						});
						if (!exists) {
					...
					$scope.updateShipper = function (li) {
						...
						if (!li) {
							...
							angular.forEach($scope.shippers, function (s) {
								if (s.Name == $scope.currentOrder.LineItems[0].ShipperName) $scope.currentOrder.Shipper = s;
							});
							...
							// at the lineitem level for multiple shipping
							angular.forEach($scope.shippers, function (s) {
								if (s.Name == li.ShipperName) li.Shipper = s;
							});
