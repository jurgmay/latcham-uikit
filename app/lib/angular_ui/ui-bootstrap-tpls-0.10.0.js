...
angular
			...
			return {
				...
				link: function (originalScope, element, attrs, modelCtrl) {
								...
								if (inputValue === modelCtrl.$viewValue && hasFocus) {
									if (matches.length > 0) {
										...
										for (var i = 0; i < matches.length; i++) {
											...
											locals[parserResult.itemName] = matchedItem; // locals[parserResult.itemName] = matches[i];
											labelText = parserResult.viewMapper(scope, locals);
											...
											labelText =
												...
												labelAddress +
												'</p>';
											labelText = labelText.replace(regPipeToNewline, '<br>').replace(regTwoCommas, ',').replace(regOneComma, ', ');
											scope.matches.push({
												label: labelText,
												model: matchedItem, // model: matches[i]
											});
										}
					...
					scope.select = function (activeIdx) {
						...
						// element[0].focus();
						document.getElementById('address_search_button').click();
					};
	...
	.filter('typeaheadHighlight', function () {
		...
		return function (matchItem, query) {
			return query ? matchItem.replace(new RegExp(escapeRegexp(query), 'gi'), '<span class="uk-text-primary">$&</span>') : matchItem;
		};
	});
...
angular.module('template/typeahead/typeahead-match.html', []).run([
	...
	function ($templateCache) {
		$templateCache.put(
			'template/typeahead/typeahead-match.html',
			'<a tabindex="-1" bind-html-unsafe="match.label | typeaheadHighlight:query" class="uk-text-secondary"></a>'
		);
	},
]);
...
angular.module('template/typeahead/typeahead-popup.html', []).run([
	...
	function ($templateCache) {
		$templateCache.put(
			'template/typeahead/typeahead-popup.html',
			"<ul class=\"uk-list uk-list-striped uk-padding-remove uk-margin-remove uk-height-max-large uk-overflow-auto ng-isolate-scope\" ng-style=\"{display: isOpen()&&'block' || 'none', top: position.top+'px', left: position.left+'px'}\">\n" +
				'    <li ng-repeat="match in matches" class="uk-padding-small" ng-class="{active: isActive($index) }" style="cursor: pointer" ng-mouseenter="selectActive($index)" ng-click="selectMatch($index)">\n' +
